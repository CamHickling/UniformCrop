...
def find_non_white_bounding_boxL(image_path, totalWidth, totalHeight, min_width=10, min_height=10):
...
def process_imagesL(input_folder, output_folder, width, height):
    ...
    for filename in os.listdir(input_folder):
        ...
        # The image is a left page if it ends with "_l.png"
        ...
        if filename.lower().endswith(("_l.png", "_l.png")):
            ...
            bounding_box = find_non_white_bounding_boxL(image_path, width, height)
...
def find_non_white_bounding_boxR(image_path, totalWidth, totalHeight, min_width=10, min_height=10):
...
def process_imagesR(input_folder, output_folder, width, height):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    ...
    for filename in os.listdir(input_folder):
        ...
        # The image is a right page if it ends with "_r.png"
        ...
        if filename.lower().endswith(("_r.png", "_r.png")):
            ...
            # Find the bounding box of the non-white part of the image and crop it
            image_path = os.path.join(input_folder, filename)
            bounding_box = find_non_white_bounding_boxR(image_pat, width, height)
            img = Image.open(image_path)
            cropped_img = img.crop(bounding_box)
...
# Process images
process_imagesL(input_folder, output_folder, desired_widthL, desired_heightL)
process_imagesR(input_folder, output_folder, desired_widthR, desired_heightR)